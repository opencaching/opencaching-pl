...
if (!$message) {
    if (isset($_POST['submit'])) {
        ...
        $allOk = true;
        if ($subject <= "") {
            $message_errnosubject = '<span class="errormsg">' . tr('mailto_lackOfSubject') . '</span>';
            $allOk = false;
        }
        if ($text <= "") {
            $message_errnotext = '<span class="errormsg">' . tr('mailto_lackOfText') . '</span>';
            $allOk = false;
        }

        if ($allOk) {
            $subject = mb_ereg_replace('{subject}', $subject, "[opencaching] " . tr('mailto_emailFrom') . " '{from_username}': {subject}");
            $subject = mb_ereg_replace('{from_username}', $from_username, $subject);


            $mailtext_email = tr('mailto_hello') . " {to_username},\n\n";
            $mailtext_email .= "'{from_username}' " . tr('mailto_fromAddress') . " {from_email} " . tr('mailto_contactsWithYou') . " $absolute_server_URI\n";
            $mailtext_email .= tr('mailto_userProfile') . $absolute_server_URI . "viewprofile.php?userid={from_userid}\n";
            $mailtext_email .= tr('mailto_respByEmail') . "\n";
            $mailtext_email .= "----------------------\n\n";
            $mailtext_email .= "{{text}}\n";


            $mailtext_anonymous = tr('mailto_hello') . " {to_username},\n\n";
            $mailtext_anonymous .= "'{from_username}' " . tr('mailto_contactsWithYou') . " $absolute_server_URI\n";
            $mailtext_anonymous .= tr('mailto_userProfile') . $absolute_server_URI . "viewprofile.php?userid={from_userid}\n";
            $mailtext_anonymous .= tr('mailto_respByOc') . "\n";
            $mailtext_anonymous .= "----------------------\n";
            ...
            tpl_redirect('mailto.php?userid=' . urlencode($userid) . '&message=' . urlencode(tr('mailto_messageSent')));
        }
    }
}
