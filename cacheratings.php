...
if ($usr == false) {
    ...
    if (XDb::xNumRows($rs) == 0) {
        ...
        while ($record = XDb::xFetchArray($rs)) {
            ...
            $thisline = mb_ereg_replace('{cachename}', htmlspecialchars($record['name'], ENT_COMPAT, 'UTF-8'),
                $thisline);
            // PowerTrail vel GeoPath icon
            if (isset($record['PT_ID'])) {
                $PT_icon = icon_geopath_small($record['PT_ID'], $record['PT_image'], $record['PT_name'],
                    $record['PT_type'], $pt_cache_intro_tr, $pt_icon_title_tr);
                $thisline = mb_ereg_replace('{GPicon}', $PT_icon, $thisline);
            } else {
                $thisline = mb_ereg_replace('{GPicon}',
                    '<img src="images/rating-star-empty.png" class="icon16" alt="" title="" />', $thisline);
            };

            $thisline = mb_ereg_replace('{userid}', urlencode($record['user_id']), $thisline);
            $thisline = mb_ereg_replace('{username}', htmlspecialchars($record['username'], ENT_COMPAT, 'UTF-8'),
                $thisline);
