...
//prepare the templates and include all neccessary
	...
	$rsGeneralStat =sql("SELECT admin,guru,hidden_count, founds_count, is_active_flag,email, password,log_notes_count, notfounds_count, username, last_login, countries.pl country, date_created, description, hide_flag FROM user LEFT JOIN countries ON (user.country=countries.short) WHERE user_id=&1",$user_id);
			...
			if ($num_caches>=5 && $num_find_caches>=5 && $user_record['guru'] ==0 && $user_id == $usr['userid'] ){
					tpl_set_var('guide_info', '<div class="content-title-noshade box-blue"><table><tr><td><img style="margin-right: 10px;margin-left:10px;" src="tpl/stdstyle/images/blue/info-b.png" alt="guide"></td><td>
...
//------------ begin created caches ---------------------------			
			...
			$hidden_event = 0;			
			$rsms=sql("SELECT cache_id, wp_oc, DATE_FORMAT(date_created,'%Y-%m-%d') data FROM caches WHERE user_id=&1 AND status <> 4 AND status <> 5 AND status <> 6 AND type <> 6 ORDER BY YEAR(`date_created`) ASC, MONTH(`date_created`) ASC, DAY(`date_created`) ASC, HOUR(`date_created`) ASC",$user_id);
