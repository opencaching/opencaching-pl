		...
		else
		{
			...
			if (mysql_num_rows($log_rs) > 0)
			{
				...
				if (($log_record['user_id'] == $usr['userid'] && $log_record['cachestatus'] != 3 && $log_record['cachestatus'] != 4 && $log_record['cachestatus'] != 6) || $usr['admin'])
				{
					...
					$cache_latitude = $log_record['cache_latitude'];
					$cache_longitude = $log_record['cache_longitude'];
					$cache_user_id = $log_record['cache_user_id'];
					$cache_hidden =$log_record['cache_hidden'];
...
// sp2ong 28.I.2010 recommendation all caches except events
				...
				$recordxy = sql_fetch_array($rscs);
				
						if ($cache_latitude == $recordxy['latitude'] && $cache_longitude==$recordxy['longitude']  )
						{
					...
					{	$lat = $coords_lat_h + round($coords_lat_min,3) / 60;
						...
						$rsck =  sql("SELECT `longitude`, `latitude` FROM `cache_moved` WHERE `log_id` IS NULL AND `cache_id`='&1'",$log_record['cache_id']);
						if (mysql_num_rows($rsck) ==0)
						{
						sql("INSERT INTO `cache_moved` (`id`, `cache_id`, `user_id`,`date`,`longitude`,`latitude`)
										 VALUES ('', '&1', '&2', '&3','&4','&5')",
										 $log_record['cache_id'], $cache_user_id,$cache_hidden,$cache_longitude,$cache_latitude);
							} 	
				...
				if ($cm_date<$logdate)
					...
					sql("UPDATE `caches` SET `last_modified`=NOW(), `longitude`='&1', `latitude`='&2' WHERE `cache_id`='&3'",  $lon, $lat, $log_record['cache_id']);
						...
						sql("INSERT INTO `cache_moved` (`id`, `cache_id`, `user_id`, `log_id`,`date`,`longitude`,`latitude`)
										 VALUES ('', '&1', '&2', '&3','&4','&5','&6')",
										 $log_record['cache_id'], $log_record['user_id'],$log_id,
