...
if(! $log_record = XDb::xFetchArray($log_rs)){
    // there is no such log!
    throw new OcException("No such log!");
    exit;
}
...
if (
    !($log_record['user_id'] == $usr['userid'] &&
        ($usr['admin'] || ($log_record['cachestatus'] != 4 &&
            $log_record['cachestatus'] != 6)))
) {
    // this log can't be edited - redirect to cache page
