    ...
    public static function call(OkapiRequest $request)
    {
        ...
        if ($tmp = $request->get_parameter('radius'))
        {
            ...
            if ($radius <= $optimization_radius)
            {
                ...
                $where_conds[] = "
                    caches.latitude >= ".Db::float_sql($center_lat - $radius_degrees)."
                    and caches.latitude <= ".Db::float_sql($center_lat + $radius_degrees)."
                ";
            }

            $radius *= 1000;  # convert from kilometers to meters
            $where_conds[] = "$distance_formula <= ".Db::float_sql($radius);
        }
