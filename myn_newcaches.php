...
if ($error == false) {
    ...
    XDb::xSql(
        'CREATE TEMPORARY TABLE local_caches' . $user_id . ' ENGINE=MEMORY
         SELECT (' . getSqlDistanceFormula($lon, $lat, $distance, 1) . ') AS `distance`,
                 `caches`.`cache_id` AS `cache_id`, `caches`.`wp_oc` AS `wp_oc`,
         ...
         WHERE `caches`.`cache_id` NOT IN (
            ...
            AND caches.status <>6 AND `longitude` > ' . ($lon - $max_lon_diff) . '
    ...
    XDb::xSql(
        ...
        ADD INDEX(`cache_id`), ADD INDEX (`wp_oc`), ADD INDEX(`type`), ADD INDEX(`name`),
        ADD INDEX(`user_id`), ADD INDEX(`date_hidden`), ADD INDEX(`date_created`)');

    $file_content = '';
    $rs = XDb::xSql(
        'SELECT `caches`.`cache_id` `cacheid`, `caches`.`cache_id` `cache_id`,
                `user`.`user_id` `userid`, `user`.`user_id` `user_id`,
                `caches`.`country` `country`, `caches`.`type` `type`,
                `caches`.`name` `cachename`, `caches`.`wp_oc` `wp_name`,
                `caches`.`type` `cache_type`, `user`.`username` `username`,
                ...
                `cache_type`.`icon_large` `icon_large`, IFNULL(`cache_location`.`adm3`,\'\') `region`,
        ...
        WHERE `caches`.`date_hidden` <= NOW()
            ...
            AND `caches`.`status` = 1
        ORDER BY IF((`caches`.`date_hidden`>`caches`.`date_created`),
