...
function outputXmlFile($sessionid, $filenr, $bXmlDecl, $bOcXmlTag, $bDocType, $ziptype)
{
	...
	if (!is_dir($zip_basedir . 'ocxml12/' . $sessionid))
		mkdir($zip_basedir . 'ocxml12/' . $sessionid);
	
	$fileid = 1;
	while (file_exists($zip_basedir . 'ocxml12/' . $sessionid . '/' . $sessionid . '-' . $filenr . '-' . $fileid . '.xml'))
		$fileid++;
		
	$xmlfilename = $zip_basedir . 'ocxml12/' . $sessionid . '/' . $sessionid . '-' . $filenr . '-' . $fileid . '.xml';
	...
	if ($bDocType == '1') fwrite($f, '<!DOCTYPE oc12xml PUBLIC "-//Opencaching Network//DTD OCXml V 1.1//EN" "http://www.opencaching.pl/xml/ocxml12.dtd">' . "\n");
	if ($bOcXmlTag == '1')
	{
		$rs = sql('SELECT `date_created`, `modified_since` FROM `xmlsession` WHERE `id`=&1', $sessionid);
		$r = sql_fetch_array($rs);
		fwrite($f, '<oc12xml version="1.1" date="' . date($sDateformat, strtotime($r['date_created'])) . '" since="' . date($sDateformat, strtotime($r['modified_since'])) . '">' . "\n");
		mysql_free_result($rs);
	}
	...
	while ($r = sql_fetch_array($rs))
	{
		...
		fwrite($f, $t2 . '<score result="' . $r['score'] . '" votes="' . $r['votes'] . '" />' . "\n");
	...
	$rs = sql('SELECT `cache_desc`.`id` `id`, `cache_desc`.`uuid` `uuid`, `cache_desc`.`cache_id` `cache_id`, 
	                  ...
	                  `cache_desc`.`desc` `desc`, `cache_desc`.`desc_html` `desc_html`, `cache_desc`.`hint` `hint`, 
										`cache_desc`.`rr_comment` `rr_comment`,
	...
	while ($r = sql_fetch_array($rs))
	{
		...
		fwrite($f, $t2 . '<rrcomment>' . xmlcdata($r['rr_comment']) . '</rrcomment>' . "\n");
	...
	if ($bOcXmlTag == '1') fwrite($f, '</oc12xml>' . "\n");
	
	fclose($f);
	
	$rel_xmlfile = 'ocxml12/' . $sessionid . '/' . $sessionid . '-' . $filenr . '-' . $fileid . '.xml';
	$rel_zipfile = 'ocxml12/' . $sessionid . '/' . $sessionid . '-' . $filenr . '-' . $fileid;

	// zippen und url-redirect
	if ($ziptype == '0')
	{
		tpl_redirect($zip_wwwdir . 'ocxml12/' . $sessionid . '/' . $sessionid . '-' . $filenr . '-' . $fileid . '.xml');
		exit;
	}
	else if ($ziptype == 'zip')
		$rel_zipfile .= '.zip';
...
function startXmlSession($sModifiedSince, $bCache, $bCachedesc, $bCachelog, $bUser, $bPicture, $bRemovedObject, $bPictureFromCachelog, $selection)
{
	...
	if ($selection['type'] == 0)
	{
		...
		{
			sql("INSERT INTO `xmlsession_data` (`session_id`, `object_type`, `object_id`)
			     SELECT &1, 1, `cache_logs`.`id` FROM `cache_logs` INNER JOIN `caches` ON `cache_logs`.`cache_id`=`caches`.`cache_id` WHERE `cache_logs`.`last_modified` >= '&2' AND `caches`.`status`!=5 AND `status`!=6 AND `status`!=4",
			     $sessionid,
			     $sModifiedSince);
