    ...
    if ($selection['type'] == 0) {
        ...
        if ($bCachelog == 1) {
            sql("INSERT INTO `xmlsession_data` (`session_id`, `object_type`, `object_id`)
                 ...
                 WHERE `cache_logs`.`cache_id`=`tmpxmlSesssionCaches`.`cache_id` AND `cache_logs`.`last_modified` >= '&2' AND `cache_logs`.`deleted`=0", $sessionid, $sModifiedSince);
            $recordcount['cachelogs'] = mysql_affected_rows();
        }
            ...
            if ($bPictureFromCachelog == 1) {
                sql("INSERT INTO `xmlsession_data` (`session_id`, `object_type`, `object_id`)
                     ...
                     WHERE `tmpxmlSesssionCaches`.`cache_id`=`cache_logs`.`cache_id` AND
                           ...
                           `pictures`.`last_modified` >= '&2' AND `cache_logs`.`deleted`=0", $sessionid, $sModifiedSince);
