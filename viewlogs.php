...
$userIsOcTeam = ($userIsLogged) ? $user->hasOcTeamRole() : false;
$userId = ($userIsLogged) ? $user->getUserId() : null;

$view->setVar('isUserAuthorized', $userIsLogged);
...
$disable_spoiler_view = (! $userIsLogged && $hide_coords); //hide any kind of spoiler if user not logged in
...
if ($cache_id != 0) {
    ...
    if ($dbc->rowCount($s) == 0) {
        ...
        if (($cache_record['status'] == 4 || $cache_record['status'] == 5 || $cache_record['status'] == 6) && ($cache_record['user_id'] != $userId && !$userIsOcTeam)) {
            $cache_id = 0;
        }
    }
} else {
...
if ($cache_id != 0) {
    ...
    if (@$enable_cache_access_logs) {
        $access_log = @$_SESSION['CACHE_ACCESS_LOG_VL_' . $userId];
        if ($access_log === null) {
            $_SESSION['CACHE_ACCESS_LOG_VL_' . $userId] = array();
            $access_log = $_SESSION['CACHE_ACCESS_LOG_VL_' . $userId];
        }
        if (@$access_log[$cache_id] !== true) {
            $dbc->multiVariableQuery(
                ...
                $cache_id, $userId, $_SERVER['REMOTE_ADDR'],
                (isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : ''),
                (isset($_SERVER['HTTP_X_FORWARDED_FOR']) ? $_SERVER['HTTP_X_FORWARDED_FOR'] : '')
            );
            $access_log[$cache_id] = true;
            $_SESSION['CACHE_ACCESS_LOG_VL_' . $userId] = $access_log;
        }
    }
    ...
    If (($HideDeleted && !$logid && !$userIsOcTeam)) { //hide deletions if (hide_deletions options is on and this is single_log call=not and user is not COG)
    ...
    foreach ($logEneries as $record) {
        ...
        if (isset($record['deleted']) && $record['deleted']) {
            if ($userIsOcTeam) {
                ...
                if ($record['type'] == 5 || $record['type'] == 10 || $record['type'] == 11) {
                    ...
                    if ($owner_id != $userId && $record['userid'] != $userId) {
                        continue;
                    }
                }
        ...
        if ($record['edit_count'] > 0) {
            ...
            if (!$userIsOcTeam && $record['edit_by_admin'] == true && $record['type'] == 12) {
...
//END: same code ->viewlogs.php / viewcache.php
        ...
        if ($record['type'] == 12 && !$userIsOcTeam) {
            $record['userid'] = '0';
        ...
        if ($record['deleted'] != 1) {
            if ($record['user_id'] == $userId && ($record['type'] != 12 || $userIsOcTeam)) {
            ...
            } elseif ($owner_id == $userId && $record['type'] != 12) {
        ...
        if (($record['picturescount'] > 0) && (($record['deleted'] == false) || ($userIsOcTeam))) { // show pictures if (any added) and ((not deleted) or (user is admin))
            ...
            while ($pic_record = $dbc->dbResultFetch($s)) {
                ...
                if ($pic_record['user_id'] == $userId || $userIsOcTeam) {
