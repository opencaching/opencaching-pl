...
{
	$query = sql("SELECT caches.longitude, caches.latitude, caches.wp_oc as wp, caches.votes, (SELECT count(*) FROM cache_logs WHERE cache_id=".$cacheid." AND type=1) as founds, (SELECT count(*) FROM cache_logs WHERE cache_id=".$cacheid." AND type=2) as notfounds, caches.topratings, caches.score as score, caches.name as cachename, user.username as username FROM caches, user WHERE user.user_id = caches.user_id AND cache_id = '".$cacheid."'");
...
function typeToImageName(type, status)
{
	...
	{
		case "1":
			return "unknown"+statusToImageName(status)+".gif";
		case "2":
		default:
			return "traditional"+statusToImageName(status)+".gif";
		case "3":
			return "multi"+statusToImageName(status)+".gif";
		case "4":
			return "virtual"+statusToImageName(status)+".gif";
		case "5":
			return "webcam"+statusToImageName(status)+".gif";
		case "6":
			return "event"+statusToImageName(status)+".gif";
		case "7":
			return "mystery"+statusToImageName(status)+".gif";
		case "8":
			return "moving"+statusToImageName(status)+".gif";
		//case "C":
		//	return "mathe"+statusToImageName(status)+".gif";
		//case "D":
		//	return "drivein"+statusToImageName(status)+".gif";
	}
}
