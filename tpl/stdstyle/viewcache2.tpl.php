		...
		<td class="header">
			<table class="null">
				<tr>
					...
					<td valign="top">Skrzynka <font size="4"><b>{cachename}</b></font><span style="font-weight:400">&nbsp;założona przez <a href="viewprofile.php?userid={userid_urlencode}">{owner_name}</a></span><br />
					{short_desc}
...
echo "";
		...
		else
		{
						...
						echo "</span><font size=\"3\"><b>"; ?>{coords} <?php echo "</b></font> <font size=\"1\">(WGS84)</font><br />
						<font size=\"1\"><a href=\"#\" onClick=\"javascript:window.open('http://www.opencaching.de/coordinates.php?lat=";?>{latitude}<?php echo "&lon=";?>{longitude}<?php echo "&popup=y&wp={oc_waypoint}','Współrzędne w innych systemach','width=240,height=334,resizable=no,scrollbars=0')\">Współrzędne w innych systemach</a></font><br />";} ?>
		...
		else
		{
						...
						function load() {
							if (GBrowserIsCompatible()) {
								...
								map.setCenter(new GLatLng(";?>{latitude}<?php echo ",";?>{longitude}<?php echo "), 7);
								var point = new GLatLng(";?>{latitude}<?php echo ",";?>{longitude}<?php echo ");
...
<?php echo"						<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\">
							<tr>
...
echo	"<tr>
	...
	<tr>
		<td>Pobierz dane:
			<a href=\"search.php?searchto=searchbycacheid&showresult=1&f_inactive=0&startat=0&cacheid=";?>{cacheid_urlencode}<?php echo "&output=gpx\" title=\"GPS Exchange Format .gpx\">GPX</a>
			<a href=\"search.php?searchto=searchbycacheid&showresult=1&f_inactive=0&startat=0&cacheid=";?>{cacheid_urlencode}<?php echo "&output=loc\" title=\"Waypoint .loc\">LOC</a>
			<a href=\"search.php?searchto=searchbycacheid&showresult=1&f_inactive=0&startat=0&cacheid=";?>{cacheid_urlencode}<?php echo "&output=kml\" title=\"Google Earth .kml\">KML</a>
			<a href=\"search.php?searchto=searchbycacheid&showresult=1&f_inactive=0&startat=0&cacheid=";?>{cacheid_urlencode}<?php echo "&output=ov2\" title=\"TomTom POI .ov2\">OV2</a>
			<a href=\"search.php?searchto=searchbycacheid&showresult=1&f_inactive=0&startat=0&cacheid=";?>{cacheid_urlencode}<?php echo "&output=ovl\" title=\"TOP50-Overlay .ovl\">OVL</a>
			<a href=\"search.php?searchto=searchbycacheid&showresult=1&f_inactive=0&startat=0&cacheid=";?>{cacheid_urlencode}<?php echo "&output=txt\" title=\"Tekst .txt\">TXT</a>
			<br />
