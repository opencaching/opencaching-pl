 ...
 * @param {String} name
 */
Garmin.WayPoint = function(lat, lng, elev, name, desc, sym, type, cmt){};
Garmin.WayPoint = Class.create();
Garmin.WayPoint.prototype = {

	/** Prototype constructor
	 */
	initialize: function(lat, lng, elev, name, desc, sym, type, cmt) {
		this.lat = lat;
		this.lng = lng;
		this.name = name;
		this.elev = elev;		
	...
	toString: function() {
		return "Wayoint: (" + this.getLat() + ", " +  this.getLng() + ")";
	}
};
...
Garmin.GpsDataFactory.prototype = {
	...
	parseGpxWaypoint: function(waypointNode) {
		...
		var wpt  = new Garmin.WayPoint(lat, lng, ele, name, desc, sym, type, cmt);		
   		return wpt;
	},
