...
Garmin.TcxActivityFactory = {
	...
	parseDocument: function(tcxDocument) {
		// create and return an array of activities from the document
		return Garmin.TcxActivityFactory._parseTcxActivities(tcxDocument);
	},
	...
	_parseTcxActivities: function(tcxDocument) {
		...
		var activityNodes = tcxDocument.getElementsByTagName(Garmin.TcxActivityFactory.SCHEMA_TAGS.activity);
		for (var i = 0; i < activityNodes.length; i++) {
			
			// create new activity object
			var activity = Garmin.TcxActivityFactory._parseTcxActivity(activityNodes[i]);
			...
			if (lapNodes.length > 0) {
				...
				if (historySeries.getSamplesLength() > 0) {				
					...
					// add the populated activity to the list of activities
	...
	_parseTcxActivity: function(activityNode) {
		// create new activity object
		...
		// set id
		var id = Garmin.TcxActivityFactory._tagValue(activityNode, Garmin.TcxActivityFactory.SCHEMA_TAGS.activityId);
