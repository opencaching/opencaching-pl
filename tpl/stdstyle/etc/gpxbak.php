...
';
	...
	while($r = sql_fetch_array($rs))
	{
		...
		$thisline = mb_ereg_replace('{cachename}', xmlentities2(PLConvert('UTF-8','POLSKAWY',$r['name'])), $thisline);
		...
		if ($r['hint'] == '')
			...
			$thisline = mb_ereg_replace('{hints}', '<hints>' . xmlentities(strip_tags(PLConvert('UTF-8','POLSKAWY',$r['hint']))) . '</hints>', $thisline);
		
		$thisline = mb_ereg_replace('{shortdesc}', xmlentities2(PLConvert('UTF-8','POLSKAWY',$r['short_desc'])), $thisline);
                $thisline = mb_ereg_replace('{desc}', xmlentities2(strip_tags(PLConvert('UTF-8','POLSKAWY',$r['desc']))), $thisline);
		...
		// logs ermitteln
		$logentries = '';
		$rsLogs = sql("SELECT `cache_logs`.`id`, `cache_logs`.`type`, `cache_logs`.`date`, `cache_logs`.`text`, `user`.`username` FROM `cache_logs`, `user` WHERE `cache_logs`.`user_id`=`user`.`user_id` AND `cache_logs`.`cache_id`=&1 ORDER BY `cache_logs`.`date` DESC", $r['cacheid']); // adam: removed LIMIT 20
		while ($rLog = sql_fetch_array($rsLogs))
		{
			...
			$thislog = mb_ereg_replace('{type}', $logtype, $thislog);
                        $thislog = mb_ereg_replace('{text}', xmlentities2(strip_tags(PLConvert('UTF-8','POLSKAWY',$rLog['text']))), $thislog);
	...
	function xmlentities($str)
	{
        ...
        {
          $str = mb_ereg_replace('&oacute;', 'o', $str);
