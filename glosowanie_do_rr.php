...
//prepare the templates and include all neccessary
	...
	$tpl_subtitle = 'Głosowanie do Rady Rejsu 2012';
		...
		else
		{
			...
			if ((time() >= $vote_time_start) && (time() <= $vote_time_end)) { // data ok - można głosować 
				...
				if (($user_num_rows == 1) && ($num_find_caches >= $user_min_found_caches)) { // użytkownik uprawiony do głosowania
					...
					$query = "SELECT * FROM rr_ocpl_vote_2012 WHERE user_id='". sql_escape($usr['userid']) . "'";
					$rs = sql($query);
					...
					if ($vote_num_rows == 0) { // użytkownik nie głosował
						...
						if( isset($_POST['glosowanie']) && ($_POST['glosowanie'] == 1)) { //został wysłany POST z głosami
							...
							$query = "SELECT candidate_id FROM rr_ocpl_candidates_2012 ORDER BY username";
							$rs = sql($query);
							...
							if (count($votes) > 0 && count($votes) < 6) { //zapisz głos
								...
								for ($i = 0; $i < count($votes); $i++) {
									sql('INSERT INTO `rr_ocpl_vote_2012` (`user_id`, `candidate_id`) VALUES (\'' . sql_escape($usr['userid']) . '\', \'' . sql_escape($votes[$i]) . '\')');
								}
	...
	function show_candidate_vote_list($candidate_vote_line) {
		$candidate_vote_list = '';
		$query = "SELECT * FROM rr_ocpl_candidates_2012 ORDER BY username";
		$rs = sql($query);
		for ($i = 0; $i < mysql_num_rows($rs); $i++)
		{
			$record = sql_fetch_array($rs);
	...
	function show_candidate_result_list($candidate_result_line) {
		...
		//$query = "SELECT count(v.candidate_id), v.candidate_id, c.username, c.city, c.user_id FROM rr_ocpl_candidates_2012 c, rr_ocpl_vote_2012 v WHERE v.candidate_id = c.candidate_id GROUP BY v.candidate_id ORDER BY count(v.candidate_id), c.username";
		$query = "SELECT count(v.candidate_id) as ilosc, c.username, c.city, c.user_id FROM rr_ocpl_candidates_2012 c LEFT JOIN rr_ocpl_vote_2012 v ON c.candidate_id = v.candidate_id GROUP BY c.username ORDER BY count(v.candidate_id) DESC, c.username";
		$rs = sql($query);
		for ($i = 0; $i < mysql_num_rows($rs); $i++)
		{
			$record = sql_fetch_array($rs);
