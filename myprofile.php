...
if ($error == false) {
    ...
    if ($usr == false) {
        ...
        tpl_set_var('registered_since', TextConverter::fixPlMonth(htmlspecialchars(strftime(
            $GLOBALS['config']['dateformat'], strtotime($record['date_created'])), ENT_COMPAT, 'UTF-8')));
                    ...
                    if (isset($_POST['submit'])) {
                        ...
                        if (!($username_not_ok ||
                            ...
                            $db->multiVariableQuery(
                                 "UPDATE `user` 
                                  SET `last_modified`=NOW(), `latitude`=:1, `longitude`=:2, 
                                      `country`=:3, `permanent_login_flag`=:4, 
                                      `power_trail_email`=:5 , `notify_radius`=:6, `ozi_filips`=:7, `guru`=:8 
                                  WHERE `user_id`=:9", 
                                  $latitude, $longitude, 
                                  $country, $using_permantent_login, 
                                  $geoPathsEmail, $radius, $ozi_path, $guide, (int) $usr['userid'] );
