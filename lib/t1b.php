<?php
  ...
  $rsU = sql('SELECT COUNT(*) `count` FROM (SELECT COUNT(caches.user_id) FROM `caches` WHERE (`status`=1 OR `status`=2 OR `status`=3) GROUP BY `user_id`) `users_with_founds`');
  $fC = sql('SELECT COUNT(*) `count` FROM `caches` WHERE (`status`=1 OR `status`=2 OR `status`=3) AND `caches`.`type`<>6');
...
COLLATE=utf8_polish_ci SELECT COUNT(*) `count`, `user`.`username`, `user`.`user_id` FROM `caches` INNER JOIN `user` ON `caches`.`user_id`=`user`.`user_id` WHERE (`caches`.`status`=1 OR `caches`.`status`=2 OR `caches`.`status`=3 ) AND `caches`.`type`<>6 AND user.stat_ban = 0 GROUP BY `user`.`user_id` ORDER BY `count` DESC, `user`.`username` ASC"; 
mysql_query("SET NAMES 'utf8'"); 
