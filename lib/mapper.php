...
while ($cache = mysql_fetch_array($query)) {
    if (!($_GET['h_u'] == "true" && $cache['type'] == 1) // hide unknown type?
            ...
            && (($cache['score'] >= $min_score && $cache['score'] <= $max_score && $cache['votes'] >= 3) || ($cache['votes'] < 3 && $_GET['h_noscore'] == "true" )) && $cache['status'] <= 3                                                                       // always hide waiting for approval, not yet available, blocked by rr
            && $_GET['h_pl'] == "true"
    ) {
