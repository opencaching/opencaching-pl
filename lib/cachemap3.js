...
$( function() { //on document ready load additional script
...
function saveMapSettings(mapTypeId)
{
    ...
    if (initial_params.savesettings === false) return;
...
function addOCOverlay()
{
    var tilelayer = new google.maps.ImageMapType({
        ...
        getTileUrl: function(tile, zoom) {
            var url=initial_params.cachemap_mapper+
                "?userid="+initial_params.userid+
                "&z="+zoom+"&x="+tile.x+"&y="+tile.y+
                prepareCommonFilterParams()+
                "&rand="+get_current_rand();

            if(initial_params.searchdata != "")
                url += "&searchdata="+initial_params.searchdata;

            if(initial_params.powertrail_ids != "")
                if( $('#pt_selection').is(':checked')) //skip to see all caches
                    url += '&powertrail_ids='+initial_params.powertrail_ids;
 ...
 */
function init_controlls( map ){
    
    if( initial_params.controls.hasOwnProperty('ctrl_combo') &&
            initial_params.controls.ctrl_combo.enabled ){
        
        var combo = 
            document.getElementById(initial_params.controls.ctrl_combo.id);
        map.controls[initial_params.controls.ctrl_combo.pos]
            .push(combo);
        
    }
    
    //init fullscreen button
    if( initial_params.controls.hasOwnProperty('fullscreen') &&
            initial_params.controls.fullscreen.enabled ){
        var button = $('#'+initial_params.controls.fullscreen.id );
        
        google.maps.event.addDomListener(button[0], "click", function() {            
            if( initial_params.fullscreen ){
                document.cookie = "forceFullScreenMap=off;"; //remember user decision in cookie
                window.location = "cachemap3.php"+getMapParamsToUrl(); 
            }else{
                document.cookie = "forceFullScreenMap=on;";
                window.location = "cachemap-full.php"+getMapParamsToUrl();                
            }            
        });
    }
    
    //init current position button
    if( initial_params.controls.hasOwnProperty('position') && 
            initial_params.controls.position.enabled ){
        
        var button = $('#'+initial_params.controls.position.id );
    ...
    if( initial_params.controls.hasOwnProperty('filters') && 
            initial_params.controls.filters.enabled ){
        
        var filters_div = $('#'+initial_params.controls.filters.box_id );
        var button = $('#'+initial_params.controls.filters.but_id );
    ...
    if ( initial_params.controls.hasOwnProperty('search') && 
            initial_params.controls.search.enabled ){
        
        var placeSearchText = document.getElementById(initial_params.controls.search.input_id);
        var placeSearchButton = document.getElementById(initial_params.controls.search.but_id);
...
function loadOcMap()
{
    ...
    map = new google.maps.Map(
        ...
        {
            center: new google.maps.LatLng(initial_params.coords[0], initial_params.coords[1]),
            zoom: initial_params.zoom,
            mapTypeId: getMapTypeFromOCMapId(initial_params.map_type),
            ...
            overviewMapControl: initial_params.largemap
        }
    );

    if (initial_params.largemap === false) {
    ...
    if (isTouchScreen) {
        ...
        if (initial_params.fullscreen) {
            ...
            if( window.innerWidth < 400) /* hide maps on small screens only */
                initial_params.moremaptypes = false;
    ...
    for (var mapType in mapItems){
        ...
        if (!initial_params.moremaptypes && (!showMapsWhenMore[mapType])
             || initial_params.moremaptypes
             || mapTypeId2 === mapType)
        {
    ...
    if (initial_params.largemap && !isTouchScreen) {
    ...
    if (initial_params.circle === 1)
    {
        // draw circle with radius 150 m to check existing geocaches
        var punktCentralny = new google.maps.LatLng(initial_params.coords[0], initial_params.coords[1]);
    ...
    if(initial_params.doopen)
        onClickFunc({ latLng: new google.maps.LatLng(initial_params.coords[0], initial_params.coords[1]) });

    if(initial_params.fromlat !== initial_params.tolat) {
        var area = new google.maps.LatLngBounds();
        area.extend(new google.maps.LatLng(initial_params.fromlat, initial_params.fromlon));
        area.extend(new google.maps.LatLng(initial_params.tolat,   initial_params.tolon));
        map.fitBounds(area);
    }


    if (initial_params.fullscreen && $.browser.msie && (parseInt($.browser.version, 10) < 8)) {
        ...
        setTimeout(function() {
            ...
            map.setCenter(new google.maps.LatLng(initial_params.coords[0], initial_params.coords[1]));
        }, 1000);
    }
...
//read current map attributs to set in url
function getMapParamsToUrl(){

    var mapUrlParams = "?lat="+map.getCenter().lat()+
                       "&lon="+map.getCenter().lng()+
                       "&inputZoom="+map.getZoom();

    if(initial_params.extrauserid != "")
        mapUrlParams += "&userid="+ initial_params.extrauserid;

    if(initial_params.boundsurl != "")
        mapUrlParams += initial_params.boundsurl;

    if(initial_params.searchdata != "")
        mapUrlParams += "&searchdata="+initial_params.searchdata;

    if(initial_params.powertrail_ids != "")
        mapUrlParams += "&pt="+initial_params.powertrail_ids;
