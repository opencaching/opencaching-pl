...
function loadOcMap(params) {
    ...
    if( canvasWidth < 730 ){
        ...
        if(params.customControls.hasOwnProperty('search')){
            ...
            params.customControls.search.but_id = false;
            $('#'+params.customControls.search.input_id).width('5em');
        }

        //hide cords display
        if(params.customControls.hasOwnProperty('cordsUnderCursor')){
            params.customControls.cordsUnderCursor = {};
        }
    }

    // thin combo + dropdown map menu > 345px
    if( canvasWidth < 345 && params.customControls.hasOwnProperty('ctrlCombo')){
        //move dropdown below
...
function initCustomControlls( map, params ){
    ...
    for ( var c in controls ) {
        ...
        case 'search':
            ...
            if( controls[c].but_id !== false ){ //handle search button
