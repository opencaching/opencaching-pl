...
function addOCOverlay()
{
    var tilelayer = new google.maps.ImageMapType({
        ...
        getTileUrl: function(tile, zoom) {
            var url=initial_params.start.cachemap_mapper+
                "?userid="+initial_params.start.userid+
                "&z="+zoom+"&x="+tile.x+"&y="+tile.y+
                prepareCommonFilterParams()+
                "&rand="+get_current_rand();
            
            if(initial_params.start.searchdata != "")
                url += "&searchdata="+initial_params.start.searchdata;
            
            if(initial_params.start.powertrail_ids != "")
                url += "&powertrail_ids="+initial_params.start.powertrail_ids;
...
function getMapParamsToUrl(){
    ...
    if(initial_params.start.extrauserid != "")
        mapUrlParams += "&userid="+ initial_params.start.extrauserid;

    if(initial_params.start.boundsurl != "")
        mapUrlParams += initial_params.start.boundsurl;    

    if(initial_params.start.searchdata != "")
        mapUrlParams += "&searchdata="+initial_params.start.searchdata;

    if(initial_params.start.powertrail_ids != "")
        mapUrlParams += "&pt="+initial_params.start.powertrail_ids;
