...
//TODO: It is global because of WMSImageMapTypeOptions func.
// It would be great to clean it out
var map = null; //global map object


//on document ready load additional scripts
$( function() { 
    var lib = document.createElement('script');
    lib.type = 'text/javascript';
    //TODO: this file should be reunificate with cachemap3.js
...
function loadOcMap(params) {
    ...
    // tricks & hacks at the end...
    if (params.fullscreen && $.browser.msie && (parseInt($.browser.version, 10) < 8)) {
        // Dirty hack for IE7 in full-screen mode only. 
        // For unknown reason map does not initially fill entire window.
        // Triggering the 'resize' event fixes this problem.
        setTimeout(function() {
            google.maps.event.trigger(map, 'resize');
            map.setCenter(new google.maps.LatLng(params.coords[0], params.coords[1]));
        }, 1000);
    }
...
{        
    /**
     * refreshRand is used to refresh map tiles
     */
    var refreshRand = {
            rand: 'r0',
            generate_new: function () {
                t = new Date();
                this.rand = "r" + t.getHours() + ":" + t.getMinutes() + ":" + Math.floor(t.getSeconds() / 10) + "0";
            },
            getCurrent: function () {
                return this.rand; 
            }
 ...
 */
function initControlls( map, params ){
    ...
    for ( var c in controls ) {
        ...
        case 'filters':
            var filters_div = $('#'+controls[c].box_id );
            var button = $('#'+controls[c].but_id );
        ...
        case 'zoom_display':
            ...
            document.getElementById( controls[c].id ).value = map.getZoom();
...
function setAttributionOnMap(map){
    ...
    if( attributionDiv == null) {
        ...
        attributionDiv = createAttributionDiv(); //from cachemap3lib.js!
...
function getMouseClickCallbacks(map, params){
    ...
    //preselected cache - map is opened with opene balon
    if(params.doopen){
        onClickFunc(
            { 
