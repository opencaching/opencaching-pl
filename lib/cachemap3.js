...
function load(additionalControls, searchElement)
{
	...
	if (searchElement) {
		...
		searchControl.draw(searchElement, new google.search.DrawOptions());
	...
	var onClickFunc = function(event) {
		...
		pendingClickRequest = jQuery.ajax({ url: prepareLibXmlMapUrl(clickBounds), timeout: pendingClickRequestTimeout, success: function(data, status, jqxhr) {
			...
			if( cache_id != "" )
			{
				...
				if (wp_check=="OP") {
					var print_list_link = "&nbsp;<a href='"+domain+"viewcache.php?cacheid=" + cache_id + "&print_list=y' target='_blank'> <img class='icon16' src='images/actions/list-add-16.png' title='"+initial_params.translation.add_clipboard+"' alt='"+initial_params.translation.add_clipboard+"'></a>"; 
				} else {
					var print_list_link="";
				};
				infoWindowContent += "<\/td><td width='10%' style='white-space: nowrap'><b>"+wp+print_list_link+"<\/b><\/td><\/tr><\/table>";
