...
class sqldebugger
{
	...
	function execute($sql)
	{
		...
		{
			$this->cancel = true;
			return mysql_query($sql, $db['dblink']);
		}
		...
		if (strtoupper(substr($sqlexplain, 0, 7)) == 'SELECT ')
		{
			// we can use EXPLAIN
			$c = 0;
			$rs = mysql_query($sqlexplain, $db['dblink']);
		...
		$bSqlExecution->start();
		$rsResult = mysql_query($sql, $db['dblink']);
		$bSqlExecution->stop();
		$bError = ($rsResult == false);
		$db['affected'] = mysql_affected_rows($db['dblink']);
		$command['affected'] = $db['affected'];

		$rs = mysql_query('SHOW WARNINGS', $db['dblink']);
		while ($r = sql_fetch_assoc($rs))
