 ...
 * Unicode Reminder メモ
 *
 * Smarty extension to load translation from mysql database
 ...
 *  - new translations strings automatically detected
 *
 * Disadvantages:
...
function smarty_prefilter_t($source, &$smarty, $resource_name)
{
	...
	smarty_prefilter_t_gettext_init($smarty, $resource_name);
...
/* if DEBUG_TRANSLATE is set, clear sys_trans_ref table
 *
 */
function smarty_prefilter_t_gettext_init(&$smarty, $resource_name)
{
	global $opt, $translate;
	
	if (isset($translate))
		$translate->initTranslation($opt['template']['style'], $resource_name);
}
