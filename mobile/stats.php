<?php
	...
	if(isset($_GET['more']) && ($_GET['more']=='1' || $_GET['more']=='2' || $_GET['more']=='3' || $_GET['more']=='4')){
		...
		$query = "select name,wp_oc,ile from caches inner join (select cache_id as yy,ile from(select cache_id,count(*) as ile from cache_rating group by cache_id)tab order by ile desc limit 5)a on yy=caches.cache_id where type!='4' and type!='5'";			
		$wynik = db_query ($query);
		$znalezione = Array();
		while($odp2=mysql_fetch_assoc($wynik)){
			$odp['name']=$odp2['name'];
