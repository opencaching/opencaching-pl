...
if ($cache_record = XDb::xFetchArray($cache_rs)) {
    ...
    if ($cache_record['user_id'] == $loggedUser->getUserId() || $loggedUser->hasOcTeamRole()) {
        ...
        $descwp_not_ok_message = $view->fieldError(tr('descwp_incorrect'));
        $type_not_ok_message = $view->fieldError(tr('type_incorrect'));
        $typewp_not_ok_message = $view->fieldError(tr('typewp_incorrect'));
        $size_not_ok_message = $view->fieldError(tr('size_incorrect'));
        $diff_not_ok_message = $view->fieldError(tr('diff_incorrect'));
        $sizemismatch_message = $view->fieldError(tr('virtual_cache_size'));
        $regionNotOkMessage = $view->fieldError(tr('region_not_ok'));
        ...
        if (! mb_ereg_match('^[0-9]{1,3}$', $coords_lat_h)) {
            ...
            $lat_not_ok = ! ((($coords_lat_h >= 0) && ($coords_lat_h < 180)));
        }

        if (is_numeric($coords_lat_min)) {
            // important: use here |=
            $lat_not_ok |= ! ((($coords_lat_min >= 0) && ($coords_lat_min < 60)));
        } else {
            $lat_not_ok = true;
        }
...
//                          $wp_desc=nl2br($wp_desc);
                //save to DB
                XDb::xSql(
                    'UPDATE `waypoints` SET `longitude`=?, `latitude`=?, `type`=?,`status`=?,
        ...
        tpl_set_var('desc_message', $descwp_not_ok ? $descwp_not_ok_message : '');
        tpl_set_var('lon_message', $lon_not_ok ? $error_coords_not_ok : '');
        tpl_set_var('lat_message', $lat_not_ok ? $error_coords_not_ok : '');
