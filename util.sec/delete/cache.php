        ...
        if ($action == 'delete')
        {
            ...
            while ($r = sql_fetch_assoc($rs))
            {
                ...
                while ($rLogPics = sql_fetch_assoc($rsLogPics))
                {
                    ...
                    sql("INSERT INTO removed_objects (`localid`, `uuid`, `type`, `removed_date`, `node`) VALUES ('&1', '&2', 6, NOW(), '&3')",
                    $rLogPics['id'], $rLogPics['uuid'], $oc_nodeid);
                }
                ...
                sql("INSERT INTO removed_objects (`localid`, `uuid`, `type`, `removed_date`, `node`) VALUES ('&1', '&2', 1,NOW(), '&3')",
                                                                                    $r['id'], $r['uuid'], $oc_nodeid);
            }
            ...
            while ($r = sql_fetch_assoc($rs))
            {
                ...
                sql("INSERT INTO removed_objects (`localid`, `uuid`, `type`, `removed_date`, `node`) VALUES ('&1', '&2', 2, NOW(), '&3')",
                    $r['cache_id'], $r['uuid'], $oc_nodeid);
