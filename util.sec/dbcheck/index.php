<?php
 ...
 /***************************************************************************
        ...
        Script zur Überprüfung und Widerherstellung der Datenbank-Konsistenz.
    ...
    ***************************************************************************/
    ...
    if ($error == true) die('Error after require(common.inc.php)');
...
<html>
    <head>
        <title>Datenbank Integritätscheck</title>
    </head>
    <body>
<?php
    ...
    while ($r = mysql_fetch_array($rs))
    {
        ...
        if (($r['desc_html'] != 0) && ($r['desc_html'] != 1))
            echo 'Desc ' . $r['id'] . ': desc_html hat einen ungültigen Wert ' . $r['desc_html'] . '<br/>';
        ...
        else
        {
            ...
            $bFound = false;
            if (strpos($desc, '<') !== false)   $bFound = true;
            if (strpos($desc, '>') !== false)   $bFound = true;

            if ($bFound == true)
                echo 'Desc ' . $r['id'] . ': Die Beschreibung enhält mögl. HTML-Tags<br/>';
        }
        ...
        $bFound = false;
        if (strpos($hint, '<') !== false)   $bFound = true;
        if (strpos($hint, '>') !== false)   $bFound = true;

        if ($bFound == true)
            echo 'Desc ' . $r['id'] . ': Der Hint enhält mögl. HTML-Tags<br/>';

        $short_desc = $r['short_desc'];

        $bFound = false;
        if (strpos($short_desc, '<') !== false) $bFound = true;
        if (strpos($short_desc, '>') !== false) $bFound = true;

        if ($bFound == true)
            echo 'Desc ' . $r['id'] . ': Die short_desc enhält mögl. HTML-Tags<br/>';
        ...
        // uuid
        if (!isuuid($r['uuid']))
            echo 'Desc ' . $r['id'] . ': uuid ungültig ' . $r['uuid'] . '<br/>';
    ...
    while ($r = mysql_fetch_array($rs))
    {
        ...
        $bFound = false;
        if (strpos($text, '<') !== false)   $bFound = true;
        if (strpos($text, '>') !== false)   $bFound = true;

        if ($bFound == true)
            echo 'Cachelog ' . $r['id'] . ': Der text enhält mögl. HTML-Tags<br/>';
        ...
        if (!isuuid($r['uuid']))
            echo 'Cachelog ' . $r['id'] . ': uuid ungültig ' . $r['uuid'] . '<br/>';
        ...
        if (($r['owner_notified'] != 0) && ($r['owner_notified'] != 1))
            echo 'Cachelog ' . $r['id'] . ': owner_notified ungültig ist=' . $r['owner_notified'] . '<br/>';
        ...
        for ($i = 0; $i < count($cache_size); $i++)
        {
            if ($cache_size[$i]['id'] == $r['id'])
            {
                ...
                if ($cache_size[$i]['de'] != $r['de'])
                    echo 'cache_size ' . $r['id'] . ': de stimmt nicht mit caches.inc.php überein<br/>';

                if ($cache_size[$i]['en'] != $r['en'])
                    echo 'cache_size ' . $r['id'] . ': en stimmt nicht mit caches.inc.php überein<br/>';
        ...
        for ($i = 0; $i < count($cache_status); $i++)
        {
            if ($cache_status[$i]['id'] == $r['id'])
            {
                ...
                if ($cache_status[$i]['de'] != $r['de'])
                    echo 'cache_status ' . $r['id'] . ': de stimmt nicht mit caches.inc.php überein<br/>';

                if ($cache_status[$i]['en'] != $r['en'])
                    echo 'cache_status ' . $r['id'] . ': en stimmt nicht mit caches.inc.php überein<br/>';
        ...
        for ($i = 0; $i < count($cache_types); $i++)
        {
            if ($cache_types[$i]['id'] == $r['id'])
            {
                ...
                if ($cache_types[$i]['short'] != $r['short'])
                    echo 'cache_type ' . $r['id'] . ': short stimmt nicht mit caches.inc.php überein<br/>';

                if ($cache_types[$i]['de'] != $r['de'])
                    echo 'cache_type ' . $r['id'] . ': de stimmt nicht mit caches.inc.php überein<br/>';

                if ($cache_types[$i]['en'] != $r['en'])
                    echo 'cache_type ' . $r['id'] . ': en stimmt nicht mit caches.inc.php überein<br/>';
    ...
    while ($r = mysql_fetch_array($rs))
    {
        ...
        if (($r['longitude'] < -180) || ($r['longitude'] > 180))
            echo 'caches ' . $r['cache_id'] . ': longitude ungültig<br/>';

        if (($r['latitude'] < -90) || ($r['latitude'] > 90))
            echo 'caches ' . $r['cache_id'] . ': latitude ungültig<br/>';
        ...
        if (($r['difficulty'] < 2) || ($r['difficulty'] > 10))
            echo 'caches ' . $r['cache_id'] . ': difficulty ungültig<br/>';

        // terrain
        if (($r['terrain'] < 2) || ($r['terrain'] > 10))
            echo 'caches ' . $r['cache_id'] . ': terrain ungültig<br/>';

        // uuid
        if (!isuuid($r['uuid']))
            echo 'caches ' . $r['cache_id'] . ': uuid ungültig ' . $r['uuid'] . '<br/>';

        // watcher
        $rsC = mysql_query('SELECT COUNT(*) count FROM cache_watches WHERE cache_id=\'' . sql_escape($r['cache_id']) . '\'', $dblink);
        $rC = mysql_fetch_array($rsC);
        if ($r['watcher'] == null) $r['watcher'] = 0;
    ...
    while ($r = mysql_fetch_array($rs))
    {
        ...
        if (($r['list_default_de'] != 0) && ($r['list_default_de'] != 1))
            echo 'countries ' . $r['country_id'] . ': list_default_de ungültig<br/>';

        if (($r['list_default_en'] != 0) && ($r['list_default_en'] != 1))
            echo 'countries ' . $r['country_id'] . ': list_default_en ungültig<br/>';

        if (($r['list_default_pl'] != 0) && ($r['list_default_pl'] != 1))
            echo 'countries ' . $r['country_id'] . ': list_default_pl ungültig<br/>';
    ...
    while ($r = mysql_fetch_array($rs))
    {
        ...
        if (($r['list_default_de'] != 0) && ($r['list_default_de'] != 1))
            echo 'languages ' . $r['id'] . ': list_default_de ungültig<br/>';

        if (($r['list_default_en'] != 0) && ($r['list_default_en'] != 1))
            echo 'languages ' . $r['id'] . ': list_default_en ungültig<br/>';

        if (($r['list_default_pl'] != 0) && ($r['list_default_pl'] != 1))
            echo 'languages ' . $r['id'] . ': list_default_pl ungültig<br/>';
    }
    mysql_free_result($rs);
        ...
        for ($i = 0; $i < count($log_types); $i++)
            if ($log_types[$i]['id'] == $r['id'])
            {
                ...
                if ($log_types[$i]['en'] != $r['en'])
                    echo 'log_types ' . $r['id'] . ': en stimmt nicht mit caches.inc.php überein<br/>';
    ...
    while ($r = mysql_fetch_array($rs))
    {
        // uuid
        if (!isuuid($r['uuid']))
            echo 'pictures ' . $r['id'] . ': uuid ungültig ' . $r['uuid'] . '<br/>';

        // url
        if (substr($r['url'], 0, 7) != 'http://')
            echo 'pictures ' . $r['id'] . ': url ungültig ' . htmlspecialchars($r['url']) . '<br/>';
        ...
        if (($r['spoiler'] != null) && ($r['spoiler'] != 0) && ($r['spoiler'] != 1))
            echo 'pictures ' . $r['id'] . ': Feld spoiler ungültig<br/>';

        // local
        if (($r['local'] != null) && ($r['local'] != 0) && ($r['local'] != 1))
            echo 'pictures ' . $r['id'] . ': Feld local ungültig<br/>';

        // unknown_format
        if (($r['unknown_format'] != null) && ($r['unknown_format'] != 0) && ($r['unknown_format'] != 1))
            echo 'pictures ' . $r['id'] . ': Feld unknown_format ungültig<br/>';

        // display
        if (($r['display'] != null) && ($r['display'] != 0) && ($r['display'] != 1))
            echo 'pictures ' . $r['id'] . ': Feld display ungültig<br/>';
    }
    mysql_free_result($rs);
    ...
    $r = mysql_fetch_array($rs);
    echo $r['count'] . ' queries die älter als 10h sind [<a href="index.php?action=clearqueries">Löschen</a>]<br/>';
    mysql_free_result($rs);
    ...
    {
        // username
        if (!preg_match('/^[a-zA-Z0-9][a-zA-Z0-9\.\-_ @äüöÄÜÖ=)(\/\\\&*+~#]{2,59}$/', $r['username']))
            echo 'user ' . $r['id'] . ': username ungültig<br/>';

        // password
        if (!preg_match('/^[a-zA-Z0-9]{32}$/', $r['password']))
            echo 'user ' . $r['id'] . ': password ungültig<br/>';

        // email
        if (!preg_match('/^[^\@]+\@[^\@]+\.\w{2,}$/', $r['email']))
            echo 'user ' . $r['id'] . ': email ungültig<br/>';

        // latitude
        if ($r['latitude'] == null) $r['latitude'] = 0;
        if (($r['latitude'] < -90) || ($r['latitude'] > 90))
            echo 'user ' . $r['id'] . ': latitude ungültig<br/>';

        // longitude
        if ($r['longitude'] == null) $r['longitude'] = 0;
        if (($r['longitude'] < -180) || ($r['longitude'] > 180))
            echo 'user ' . $r['id'] . ': longitude ungültig<br/>';

        // was_loggedin

        if (($r['was_loggedin'] != null) && ($r['was_loggedin'] != 0) && ($r['was_loggedin'] != 1))
            echo 'user ' . $r['id'] . ': was_loggedin ungültig<br/>';
        ...
        // uuid
        if (!isuuid($r['uuid']))
            echo 'user ' . $r['id'] . ': uuid ungültig ' . $r['uuid'] . '<br/>';
        ...
        $rC = mysql_fetch_array($rsC);
        if ($r['hidden_count'] == null) $r['hidden_count'] = 0;
        ...
        $rC = mysql_fetch_array($rsC);
        if ($r['cache_watches'] == null) $r['cache_watches'] = 0;
