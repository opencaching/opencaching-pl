  ...
  //prepare the templates and include all neccessary
	...
	if ($error == false)
	{
		...
		else
		{
			...
			tpl_set_var('mandatory_field', $language[$lang]['mandatory_field']);
			tpl_set_var('full_description', $language[$lang]['full_description']);
			tpl_set_var('full_desc_long_text', $language[$lang]['full_desc_long_text']);
			tpl_set_var('description_hint', $language[$lang]['description_hint']);
			tpl_set_var('no_javascript', $language[$lang]['no_javascript']);
			tpl_set_var('html_usage', $language[$lang]['html_usage']);
			tpl_set_var('available_html', $language[$lang]['available_html']);
			tpl_set_var('additional_enc_info', $language[$lang]['additional_enc_info']);
			tpl_set_var('other', $language[$lang]['other']);
			tpl_set_var('add_photo_newcache', $language[$lang]['add_photo_newcache']);
			...
			tpl_set_var('submit_new_cache', $language[$lang]['submit_new_cache']);
			tpl_set_var('publish_now', $language[$lang]['publish_now']);
			tpl_set_var('publish_date', $language[$lang]['publish_date']);
			tpl_set_var('hour', $language[$lang]['hour']);
			tpl_set_var('dont_publish_yet', $language[$lang]['dont_publish_yet']);
			...
			tpl_set_var('password_info_label', $language[$lang]['password_info_label']);
			tpl_set_var('creating_cache', $language[$lang]['creating_cache']);
			tpl_set_var('rules', $language[$lang]['rules']);
			tpl_set_var('with_all_consequences', $language[$lang]['with_all_consequences']);
...
*/
				...
				if (!($name_not_ok || $hidden_date_not_ok || $activation_date_not_ok || $lon_not_ok || $lat_not_ok || $desc_html_not_ok || $time_not_ok || $way_length_not_ok || $size_not_ok || $type_not_ok || $diff_not_ok))
				{
					...
					if($publish == 'now' || ($publish == 'later' && mktime($activate_hour, 0, 0, $activate_month, $activate_day, $activate_year) <= $today))
					{
						//do event handling
