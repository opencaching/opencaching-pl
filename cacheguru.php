...
//user logged in?
	...
	$uLon = sqlValue("SELECT `user`.`longitude`  FROM `user` WHERE `user_id`='". sql_escape($usr[userid]) ."'",0);
			...
			$rscp = sql("SELECT `user`.`latitude` `latitude`,`user`.`longitude` `longitude`,`user`.`username` `username`,
					...
					FROM `user`
					WHERE `user`.`guru`!=0
					AND `user`.`longitude` IS NOT NULL AND `user`.`latitude` IS NOT NULL");
			$point="";
			...
			$point .="GEvent.addListener(marker".$number.", \"click\", function() {marker".$number.".openInfoWindowHtml('<span style=\"font-size:12px;color:blue;\"><br/><p><img src=\"tpl/stdstyle/images/free_icons/book_open.png\" alt=\"img\"> <b>Przewodnik/Guru: <a href=\"viewprofile.php?userid=".$record['userid']."\">".$username."</a></p><p><img src=\"tpl/stdstyle/images/free_icons/email.png\" alt=\"img\"> Kontakt: <a href=\"mailto.php?userid=".$record['userid']."\">Napisz E-mail</a></b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p></span>&nbsp;&nbsp;&nbsp;<br/>');});\n bounds.extend(point);\n";
			}
