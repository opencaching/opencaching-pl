...
//prepare the templates and include all neccessary
	...
	if ($error == false)
	{
		...
		else
		{
			...
			tpl_set_var('cache_name', tr('for_cache').': <font color="black">'.$cache_record['cache_name'].'</font>');

			tpl_set_var('notes_links', '<br /><img src="tpl/stdstyle/images/free_icons/add.png" title=""/>&nbsp;<a class="links" href="new_cachenotes.php?cacheid='.$cache_id.'">'.tr('add_new_note').'</a> &nbsp;&nbsp;<img src="tpl/stdstyle/images/free_icons/table_go.png" title=""/>&nbsp;<a class="links" href="viewcache.php?cacheid='.$cache_id.'">'.tr('back_to_cache').'</a><br /><br />');
			...
			$notes_rs = sql("SELECT `cache_notes`.`note_id` `note_id`, `cache_notes`.`cache_id` `cacheid`,`cache_notes`.`date` `date`, `cache_notes`.`desc` `desc`, `cache_notes`.`desc_html` `desc_html`, `caches`.`name` `cache_name`, `cache_type`.`icon_small` `icon_large` FROM `cache_notes` INNER JOIN caches ON (`caches`.`cache_id` = `cache_notes`.`cache_id`), `cache_type`  WHERE (`cache_notes`.`user_id`=&1 $cache_param) AND `cache_type`.`id`=`caches`.`type` ORDER BY `cacheid`,`date` DESC",$userid);
				...
				{	
						...
						$notes .= '<tr><th width="40"><b>Data</b></th><th><b>'.tr('cache_notes').'</b></th><th width="22"><b>'.tr('edit').'</b></th><th width="22"><b>'.tr('delete').'</b></th></tr>';
			...
			{	
						...
						$notes .= '<tr><td width="22"><strong>'.tr('cache_notes').'</strong></td><td><strong>#</strong></td><td width="22">&nbsp;</td><td><strong>Cache</strong></td></tr><tr><td colspan="4"><hr></hr></td></tr>';
						...
						for ($i = 0; $i < mysql_num_rows($notes_rs); $i++)
							...
							$notes .= '<tr><td><a class="links" href="mycache_notes.php?cacheid='.$notes_record['cacheid'].'"><img src="tpl/stdstyle/images/free_icons/note_edit.png" alt="" title="Pokaż notatki" /></a></td><td width="22"><strong>'.$notes_record['count']. '</strong></td><td width="22"><img src="tpl/stdstyle/images/'.$notes_record['icon_large'].'" alt="" /></td><td align="left" valign="middle"><a class="links" href="viewcache.php?cacheid='.$notes_record['cacheid'].'">'.$notes_record['cache_name'].'</a></td></tr>';
							}
