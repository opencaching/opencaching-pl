...
//		{
			...
			$rs_caches = sql("	SELECT  `caches`.`cache_id`, `caches`.`name`, `caches`.`date_hidden`, DATE_FORMAT(`caches`.`date_activate`,'%Y-%m-%d'), `caches`.`status`, `cache_status`.`&1` AS `cache_status_text`
...
//						$usr['userid']);
		...
		//get waiting to approve caches by OC Team
			...
			$rs_caches = sql("	SELECT	`cache_id`, `name`, DATE_FORMAT(`date_hidden`,'%Y-%m-%d'), `status`,
			...
			if (mysql_num_rows($rs_caches) != 0)
			{
				...
				for ($i = 0; $i < mysql_num_rows($rs_caches); $i++)
				{
					...
					$tmp_cache = mb_ereg_replace('{date}', $record_logs['date_hidden'], $tmp_cache);
