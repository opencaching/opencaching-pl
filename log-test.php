...
//				mysql_result($is_scored_query,0);
			...
			$rscmxy = sql("SELECT `cache_moved`.`latitude` `latitude`,
			                   `cache_moved`.`longitude` `longitude`,`cache_moved`.`date` `date`
								...
								AND `cache_moved`.`log_id` IS NOT NULL 	
			         ORDER BY `cache_moved`.`date` DESC LIMIT 1", $record['cache_id']);
			...
			$cm_date=strtotime($rcmxy['date']);
				...
				else
				{
					...
					foreach ($log_types AS $type)
					{
						...
						{
							if ($log_type==3) {
							$logtypeoptions .= '<option value="3" selected="selected">'.tr('comment_log').'</option>' . "\n";} else {
							$logtypeoptions .= '<option value="3">'.tr('comment_log').'</option>' . "\n";}

							if  ($res2['type'] == 8){
								if ($log_type==4) {							
							$logtypeoptions .= '<option value="4" selected="selected">'.tr('moved_log').'</option>' . "\n";} else{ 
							$logtypeoptions .= '<option value="4">'.tr('moved_log').'</option>' . "\n";}}
							$logtypeoptions .= '<option value="5">'.tr('need_maintenance_log').'</option>' . "\n";
							if ($usr['admin']==true){$logtypeoptions .= '<option value="12">'.tr('comment_octeam_log').'</option>' . "\n";}
							break;
						}
