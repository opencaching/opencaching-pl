...
$rs = XDb::xSql(
    'SELECT `caches`.`cache_id` `cacheid`, `user`.`user_id` `userid`,
                ...
                `cache_type`.`icon_small` `icon_small`, cache_location.adm3 AS state
...
if (isset($newcaches)) {
    foreach ($newcaches AS $statename => $state_record) {
        $cache_location = '<tr><td colspan="7" class="content-title-noshade-size1">'.htmlspecialchars($statename,
                ENT_COMPAT, 'UTF-8').'</td></tr>';
        ...
        foreach ($state_record AS $cache_record) {
            ...
            $file_content .= '<td><b><a class="links" href="viewcache.php?cacheid='.htmlspecialchars($cache_record['cache_id'],
                    ...
                    'UTF-8').'</a></b></td>';
            $file_content .= '<td style="width: 32px;"><b><a class="links" href="viewprofile.php?userid='.htmlspecialchars($cache_record['userid'],
                    ENT_COMPAT, 'UTF-8').'">'.htmlspecialchars($cache_record['username'], ENT_COMPAT,
                    'UTF-8').'</a></b></td>';
            ...
            if ($r_log = XDb::xFetchArray($rs_log)) {
                ...
                $file_content .= '<td style="width: 80px;">'.htmlspecialchars(date("d-m-Y",
                        strtotime($r_log['log_date'])), ENT_COMPAT, 'UTF-8').'</td>';

                $file_content .= '<td style="width: 22px;"><b><a class="links" href="viewlogs.php?logid='.htmlspecialchars($r_log['id'],
