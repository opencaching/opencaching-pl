    ...
    private function recommendationsTable($userId, $isRemovingAllowed)
    {
        ...
        if ($rowCount > 0) {
            ...
            $model->addColumn(new Column_CacheName(
                tr('myRecommendations_cacheName'),
                function ($row) {
                    return [
                        'cacheWp' => $row['wp_oc'],
                        'cacheName' => $row['name'],
            ...
            $model->addColumn(new Column_UserName(
                tr('myRecommendations_cacheOwner'),
                function ($row) {
                    return [
                        'userId' => $row['user_id'],
            ...
            if ($isRemovingAllowed) {
                $model->addColumn(new Column_OnClickActionIcon(
                    tr('myRecommendations_actionRemove'),
                    function ($row) {
                        return [
