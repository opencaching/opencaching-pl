...
class CacheAdoptionController extends BaseController
{
    ...
    public function accept($cacheId)
    {
        ...
        $this->db->multiVariableQuery(
            'DELETE FROM chowner WHERE cache_id = :1',
            $cacheObj->getCacheId()
        );
        ...
        EmailSender::sendAdoptionSuccessMessage(
            __DIR__ . '/../../resources/email/adoption.email.html',
            $cacheObj->getCacheName(),
    ...
    public function refuse($cacheId)
    {
        ...
        if (! is_null($oldOwner)) {
            ...
            EmailSender::sendAdoptionRefusedMessage(
                __DIR__ . '/../../resources/email/adoption.email.html',
                $cacheObj->getCacheName(),
                $this->loggedUser->getUserName(),
                $oldOwner->getUserName(),
    ...
    public function addAdoptionOffer($cacheId)
    {
        ...
        if ($newUserObj->getUserId() == $cacheObj->getOwnerId()) {
            ...
            $stmt = $this->db->multiVariableQuery(
                ...
                $cacheObj->getCacheId(),
                $newUserObj->getUserId()
            );

            if ($this->db->rowCount($stmt) > 0) {
                EmailSender::sendAdoptionOffer(
                    __DIR__ . '/../../resources/email/adoption.email.html',
                    $cacheObj->getCacheName(),
                    $newUserObj->getUserName(),
                    $this->loggedUser->getUserName(),
                    $newUserObj->getEmail()
                );
