...
<table width="760" class="table mob-w100-important mob-ml-15px mob-mr-15px" style="line-height: 1.6em; font-size: 10px;">
    <tr>
        <td>
<?php
$userscount = XDb::xSimpleQueryValue(
    'SELECT COUNT( DISTINCT user_id) FROM cache_logs WHERE type=1 AND `deleted`=0',
    0
);

$cachelogscount = XDb::xSimpleQueryValue(
    'SELECT COUNT(*) FROM `cache_logs` WHERE type=1 AND `deleted`=0',
    0
);
