...
<div class="content2-container">
    ...
    <p>
      ...
      <?= Formatter::dateTime($user->getLastLoginDate()) ?>
        ...
        <a href="#" class="js-oc-copy-to-clipboard" data-copy-to-clipboard="<?= $user->getEmail() ?>" width="16px"
           height="16px">
    ...
    <?php if ($user->isUserActivated()) { ?>
        ...
        <?php if ($user->getNotifyCaches()) { ?>
            <a href="<?= SimpleRouter::getLink('Admin.UserAdmin', 'notifyCaches', [$user->getUserId(), "0"]) ?>">
                <img src="/images/misc/on.svg" alt="<?= tr('yes') ?>" title="<?= tr('yes') ?>"
                     class="admin-onoff-switch"></a>
        <?php } else { ?>
            <a href="<?= SimpleRouter::getLink('Admin.UserAdmin', 'notifyCaches', [$user->getUserId(), "1"]) ?>">
                <img src="/images/misc/off.svg" alt="<?= tr('no') ?>" title="<?= tr('no') ?>"
                     class="admin-onoff-switch"></a>
        <?php } // if ?>
                ...
                foreach ($view->userNotes as $note) { ?>
                    <tr>
                        <td>
                            ...
                            - <a class="links"
                                 href="<?= $note->getAdmin()->getProfileUrl() ?>"><?= $note->getAdmin()->getUserName() ?></a>
                        </td>
                        </td>
                        <td>
                            <?php if ($note->isAutomatic()) { ?>
                                <img title="<?= tr("admin_notes_auto") ?>" alt="" class="icon16"
                                     src="<?= $note->getAutomaticPictureUrl() ?>">
                                <?= tr($note->getContentTranslationKey()) ?>
                                <?php if (!empty($note->getCacheId())) { ?>
                                    <a class="links"
                                       href="<?= $note->getCache()->getCacheUrl() ?>"><?= $note->getCache()->getCacheName() ?>
                                        (<?= $note->getCache()->getGeocacheWaypointId() ?>)</a>
                                <?php } // if (! empty($note->getCacheId())) ?>
                            <?php } else { //if ($note->isAutomatic()) ?>
                                <img title="<?= tr("admin_notes_man") ?>" alt="" class="icon16"
                                     src="<?= $note->getAutomaticPictureUrl() ?>">
                                <?= $note->getContent() ?>
                            <?php } ?>
                        </td>
    ...
    <?php } else { ?>
        ...
        <div class="callout callout-warning align-center">
            ...
            <a href="<?= SimpleRouter::getLink('Admin.UserAdmin', 'activateUser', $user->getUserId()) ?>"
               class="btn btn-md btn-default"><?= tr('activate_mail_btn') ?></a>
        </div>
