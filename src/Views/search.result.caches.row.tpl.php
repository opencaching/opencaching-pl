gct.addEmptyRow();
gct.addToLastRow( 0, "{CacheID}" );
gct.addToLastRow( 1, "<img span='{cachetype}' src='{icon_large}' alt='{cachetype}' title='{cachetype}' height='16' width='16' />" );
gct.addToLastRow( 2, "<a span='{cachenameBIG}' href='viewcache.php?cacheid={urlencode_cacheid}' target='_blank' class='links' style='{style}'>{cachename}</a>" );
gct.addToLastRow( 3, "{short_desc}" );
gct.addToLastRow( 4, "<a span='{usernameBIG}' href='viewprofile.php?userid={urlencode_userid}' target='_blank'  class='links'>{username}</a>" );
gct.addToLastRow( 5, "<span {date_created_sort} />{date_created}" );
gct.addToLastRow( 6, "{logtypes1}" );

gct.addToLastRow( 7, "{find}" );
gct.addToLastRow( 8, "{notfind}" );
gct.addToLastRow( 9, "{comment}" );

gct.addToLastRow( 10, "{toprating}" );
gct.addToLastRow( 11, "{logimage2}" );

gct.addToLastRow( 12, "{logtype}" );
gct.addToLastRow( 13, "{logdate}" );
gct.addToLastRow( 14, "{logdesc}" );

gct.addToLastRow( 15, "{lat}<br>{long}" );
gct.addToLastRow( 16, "<span {distance_pad} />{direction}&nbsp;{distance}" );

gct.addToLastRow( 17, "{diffpic}{terrpic}" );

gct.addToLastRow( 18, "{wp_oc}" );


